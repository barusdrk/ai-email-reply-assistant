...
export default function ReplyCard({ reply, policyCheck, loading = false, onApprove, onSend }: ReplyCardProps) {
  ...
  return (
    <div className="rounded-lg border border-(--border) bg-(--surface) shadow-sm">
      ...
      <div className="whitespace-pre-wrap px-6 py-5 text-(--text)">{reply}</div>
      {policyCheck && (
        <div className="border-t border-(--border) px-6 py-5">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="font-semibold text-(--text)">Policy Check</h3>
              <p className="text-sm text-(--text-secondary)">AI-generated reply compliance review.</p>
            </div>
          ...
          {policyCheck.violations.length > 0 && (
            <div className="mb-4 rounded-lg border border-(--error)/30 bg-(--error)/5 p-4">
              ...
              <ul className="list-disc space-y-1 pl-5 text-sm text-(--text)">
                {policyCheck.violations.map((violation, index) => <li key={`${violation}-${index}`}>{violation}</li>)}
              </ul>
            </div>
          )}
          {policyCheck.warnings.length > 0 && (
            <div className="mb-4 rounded-lg border border-(--warning)/30 bg-(--warning)/5 p-4">
              ...
              <ul className="list-disc space-y-1 pl-5 text-sm text-(--text)">
                {policyCheck.warnings.map((warning, index) => <li key={`${warning}-${index}`}>{warning}</li>)}
              </ul>
            </div>
          )}
          {policyCheck.suggestions.length > 0 && (
            <div className="rounded-lg border border-(--border) bg-(--bg-secondary) p-4">
              ...
              <ul className="list-disc space-y-1 pl-5 text-sm text-(--text-secondary)">
                {policyCheck.suggestions.map((suggestion, index) => <li key={`${suggestion}-${index}`}>{suggestion}</li>)}
              </ul>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-wrap gap-3 border-t border-(--border) px-6 py-4">
        ...
        {onApprove && (
          <button type="button" onClick={onApprove} disabled={Boolean(policyCheck && !canProceed)} className="rounded bg-(--success) px-4 py-2 text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">Approve</button>
        )}
        {onSend && (
          <button type="button" onClick={onSend} disabled={Boolean(policyCheck && !canProceed)} className="rounded bg-(--info-text) px-4 py-2 text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">Send Email</button>
        )}
      </div>
      {hasViolations && (
