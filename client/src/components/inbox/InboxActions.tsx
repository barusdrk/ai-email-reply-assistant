...
export default function InboxActions({ syncing, loadingSamples, onSync, onLoadSamples }: Props) {
  ...
  return (
    <div className="space-y-3">
      <button type="button" onClick={onSync} disabled={disabled} className="w-full rounded-lg bg-(--accent) px-4 py-2 font-medium text-(--accent-contrast) transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
