...
import { getInbox, loadSampleEmails, syncInbox, type ApiEmail } from "../services/emails.js";
...
export function useInboxPage() {
  ...
  const loadPage = useCallback(async (nextPage: number, append = false) => {
    ...
    const nextEmails = (data.emails ?? []).map(mapEmail).filter((email) => Boolean(email.id));

    setEmails((current) => {
      if (!append) return nextEmails;
      const ids = new Set(current.map((email) => email.id));
      return [...current, ...nextEmails.filter((email) => !ids.has(email.id))];
    });
  ...
  const loadMore = useCallback(async () => {
    if (loading || syncing || loadingSamples || loadingMore || !hasMore) return;
    setLoadingMore(true);
    try {
  ...
  }, [hasMore, loadPage, loading, loadingMore, loadingSamples, page, syncing]);
