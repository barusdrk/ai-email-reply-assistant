...
import { approvals, approval, requestApproval, approve, reject } from "../services/approval.js";
...
router.get("/:id", async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
  try {
    const item = await approval(req.params.id);
    if (!item) {
...
router.post("/", async (req: Request<{}, {}, { draftId: string; reviewerId: string }>, res: Response, next: NextFunction) => {
  try {
    const item = await requestApproval(req.body.draftId, req.body.reviewerId);
