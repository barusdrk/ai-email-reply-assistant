...
router.get("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({message: error instanceof Error ? error.message : "Failed to load drafts."});
  }
});
...
router.get("/:id", async (req: Request<IdParams>, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({message: error instanceof Error ? error.message : "Failed to load draft."});
  }
});
...
router.post("/", async (req: Request, res: Response) => {
  ...
  try {
    res.status(201).json(await createDraft({...req.body, userId: req.user.id}));
  } catch (error) {
    res.status(400).json({message: error instanceof Error ? error.message : "Failed to create draft."});
  }
});
...
router.put("/:id", async (req: Request<IdParams>, res: Response) => {
  ...
  try {
    ...
    res.status(400).json({message: error instanceof Error ? error.message : "Failed to update draft."});
  }
});
...
router.post("/:id/submit", async (req: Request<IdParams>, res: Response) => {
  ...
  try {
    ...
    res.status(400).json({message: error instanceof Error ? error.message : "Failed to submit draft."});
  }
});
...
router.post("/:id/approve", async (req: Request<IdParams>, res: Response) => {
  ...
  try {
    ...
    if (message.startsWith("Policy check failed:")) {
      res.status(422).json({message, policyCheckFailed: true});
      return;
    }
...
router.post("/:id/reject", async (req: Request<IdParams>, res: Response) => {
  ...
  try {
    const result = await rejectDraft(req.params.id, req.user.id, typeof req.body?.reason === "string" ? req.body.reason : undefined);
    if (!result) {
      res.status(404).json({message: "Draft not found."});
      return;
    }
    ...
    res.status(400).json({message: error instanceof Error ? error.message : "Failed to reject draft."});
  }
});
...
router.post("/:id/send", async (req: Request<IdParams>, res: Response) => {
  ...
  try {
    ...
    const result = await sendDraft(req.params.id);
    res.json(result);
  } catch (error) {
...
router.delete("/:id", async (req: Request<IdParams>, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({message: error instanceof Error ? error.message : "Failed to delete draft."});
  }
});
