...
export async function scoreReplyConfidence(
  ...
  if (!email) {
    throw new Error("Customer email is required for confidence scoring.");
  }

  if (!reply) {
    throw new Error("Reply is required for confidence scoring.");
  }
...
${knowledgeBase || "No relevant Knowledge Base information was found."}`;
  ...
  const response = await getClient().chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are a precise confidence scorer. Return only valid JSON.",
      },
      {
        role: "user",
        content: prompt,
  ...
  if (!content) {
    throw new Error("Confidence scorer returned an empty response.");
  }
