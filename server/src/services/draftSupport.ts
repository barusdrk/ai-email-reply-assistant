...
import {buildCustomerContext, buildCustomerContextFromCustomer, getCustomerContextFromEmail, mergeCustomerContext} from "./customerContext.js";
...
export async function getDraftSupportContext(userId: string, emailId: string) {
  ...
  const customerContext = mergeCustomerContext(persistedCustomerContext, emailCustomerContext);
  return {
    sourceEmail,
    customerEmailBody,
    knowledgeBase,
...
export async function analyzeDraftSupport(
  ...
  const customerContext = mergeCustomerContext(context.customerContext, suppliedCustomerContext);
