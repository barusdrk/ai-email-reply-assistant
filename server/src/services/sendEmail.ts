...
async function sendGmailEmail(options: SendEmailOptions) {
  ...
  if (!account?.accessToken) throw new Error("Gmail account is not connected.");
...
async function resolveProvider(userId: string, provider: Provider): Promise<"gmail" | "outlook"> {
  ...
  if (activeProvider !== "gmail" && activeProvider !== "outlook") {
    throw new Error("Select Gmail or Outlook as your active email provider.");
  }
  ...
  if (!account?.accessToken) {
    throw new Error(`${activeProvider === "gmail" ? "Gmail" : "Outlook"} account is not connected.`);
  }
...
export async function sendEmail(options: SendEmailOptions) {
  ...
  if (provider === "gmail") return sendGmailEmail(resolvedOptions);
