...
export type {SupportTone, SupportLength, ConversationMessage, KnowledgeBaseItem, CustomerContext, SupportPolicy, SupportEngineInput, SupportEngineResult};
export {buildPrompt, normalizeResult, DEFAULT_TONE, DEFAULT_LENGTH, CONFIDENCE_THRESHOLD, MAX_MESSAGE_LENGTH};
...
export async function analyzeSupportRequest(input: SupportEngineInput): Promise<SupportEngineResult> {
  const customerMessage = typeof input.customerMessage === "string" ? input.customerMessage.trim().slice(0, MAX_MESSAGE_LENGTH) : "";
...
export async function generateSupportReply(input: SupportEngineInput): Promise<SupportEngineResult> {
  ...
  if (result.decision !== "reply" || result.needsHuman) return {...result, reply: ""};
  if (!result.reply) {
    return {
      ...result,
