...
import {CONFIDENCE_THRESHOLD,buildPrompt,normalizeResult} from "../services/supportEngineLogic.js";
...
describe("supportEngine logic",()=>{
  ...
  it("requires customer replies to stay grounded in supplied information",()=>{
    const prompt=buildPrompt({
      customerMessage:"How do I update my account information?",
      knowledgeBase:[{
        title:"Updating Account Information",
        content:"Customers can update their account information from the Account Settings page. Sign in, open Account Settings, select Profile, select Edit, update the information, and save the changes.",
      }],
    });
    expect(prompt).toContain("Use the supplied knowledge base as the primary source");
    expect(prompt).toContain("Every factual instruction in the customer reply must be directly supported");
    expect(prompt).toContain("Never invent policies");
    expect(prompt).toContain("interface elements");
    expect(prompt).toContain("button names");
    expect(prompt).toContain("menu names");
    expect(prompt).toContain("icons");
    expect(prompt).toContain("URLs");
  });

  it("requires unsupported interface details to be excluded from knowledge-base procedures",()=>{
    const prompt=buildPrompt({
      customerMessage:"How do I update my account information?",
      knowledgeBase:[{
        title:"Updating Account Information",
    ...
    expect(prompt).toContain("Do not add alternative navigation");
    expect(prompt).toContain("interface details");
    expect(prompt).toContain("labels");
    expect(prompt).toContain("buttons");
    expect(prompt).toContain("icons");
    expect(prompt).toContain("Do not transform a general instruction into a more specific UI instruction");
    expect(prompt).toContain("plausible but unsupported details");
  });
});
